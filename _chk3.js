...
async function loadAllData(){
  ...
  STATE.files = files.map(r=>({
    ...
    fileName:r.file_name, fileSize:r.size, downloads:r.downloads||0, file_path:r.file_path,
...
async function afterLogin(){
  ...
  if(!prof){
    ...
    prof={id:user.id,name:meta.name||user.email,year:meta.year||'',is_admin:false,banned:false};
  }
  SESSION={userId:user.id,name:prof.name||user.email,email:user.email,year:prof.year||'',isAdmin:!!prof.is_admin,banned:!!prof.banned};
  enterApp();
}
...
async function doUpload(){
  ...
  try{
    ...
    const {error:insErr}=await SB.from('files').insert({title,description:desc,type,ders,uploader_id:SESSION.userId,uploader_name:SESSION.name,file_path:path,file_name:selFile.name,size:selFile.size});
