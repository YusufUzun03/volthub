...
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJncXVkcm10YmdkcnJ0Z2FhdXZvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA3NzE3NjYsImV4cCI6MjA5NjM0Nzc2Nn0.auL0BYdgRgEuR6wUjWIdtJxIpfCCKetiztQfpqgsYMQ';
const SB = supabase.createClient(SUPABASE_URL, SUPABASE_ANON);

let STATE = { files:[], profiles:[], myProfile:null };

async function loadAllData(){
 try{
  const [fR,lR,sR,rR,cR,pR] = await Promise.all([
    SB.from('files').select('*'),
    SB.from('likes').select('*'),
    SB.from('saves').select('*'),
    SB.from('ratings').select('*'),
    SB.from('comments').select('*').order('created_at',{ascending:true}),
    SB.from('profiles').select('*'),
  ]);
  const files=fR.data||[], likes=lR.data||[], saves=sR.data||[], ratings=rR.data||[], comments=cR.data||[], profiles=pR.data||[];
  const likeBy={}, rateBy={}, comBy={}, saveByUser={};
  likes.forEach(l=>(likeBy[l.file_id]=likeBy[l.file_id]||[]).push(l.user_id));
  ratings.forEach(r=>(rateBy[r.file_id]=rateBy[r.file_id]||[]).push({u:r.user_id,s:r.score}));
  comments.forEach(c=>(comBy[c.file_id]=comBy[c.file_id]||[]).push({userId:c.user_id,name:c.user_name,text:c.body,ts:new Date(c.created_at).getTime()}));
  saves.forEach(s=>(saveByUser[s.user_id]=saveByUser[s.user_id]||[]).push(s.file_id));
  STATE.files = files.map(r=>({
    id:r.id, title:r.title, desc:r.description, type:r.type, ders:r.ders,
    uploaderId:r.uploader_id, uploaderName:r.uploader_name,
    createdAt:new Date(r.created_at).getTime(),
    fileName:r.file_name, fileSize:r.size, downloads:r.downloads||0, file_path:r.file_path, tags:r.tags||[],
    likes:likeBy[r.id]||[], ratings:rateBy[r.id]||[], comments:comBy[r.id]||[]
  }));
  STATE.profiles = profiles.map(p=>({ id:p.id, name:p.name, year:p.year, downloads:p.downloads||0, is_admin:p.is_admin, banned:p.banned, saves:saveByUser[p.id]||[] }));
  STATE.myProfile = STATE.profiles.find(p=>p.id===(SESSION&&SESSION.userId)) || null;
 }catch(err){ console.error('loadAllData',err); toast('⚠️ Veriler yüklenemedi — bağlantını kontrol et.'); }
}
// Önbellek erişimcileri — eski çağrıları bozmadan çalışır
async function dbAll(s){ return s==='files'?STATE.files:STATE.profiles; }
async function dbGet(s,k){ return s==='files'?STATE.files.find(f=>f.id===k):STATE.profiles.find(p=>p.id===k); }
...
async function doLogin(){
  ...
  if(error){showErr(err,'E-posta veya şifre hatalı.');return;}
  await afterLogin();
}
...
async function doRegister(){
  ...
  const {data,error}=await SB.auth.signUp({email,password:pass,options:{data:{name,year}}});
  if(error){showErr(err,error.message);return;}
  if(data.session){ await afterLogin(); }
  else { showErr(err,'Kayıt alındı! E-postana gelen doğrulama linkine tıkla, sonra giriş yap.'); switchTab('login'); }
}
...
async function afterLogin(){
  ...
  if(!prof){
    ...
    await SB.from('profiles').insert({id:user.id,name:meta.name||'',year:meta.year||''});
    prof={id:user.id,name:meta.name||user.email,year:meta.year||'',is_admin:false,banned:false,avatar:'a1'};
  }
  SESSION={userId:user.id,name:prof.name||user.email,email:user.email,year:prof.year||'',isAdmin:!!prof.is_admin,banned:!!prof.banned,avatar:prof.avatar||'a1'};
  enterApp();
}
...
function enterApp(){
  ...
  const _av=SESSION.avatar||'a1';
  const _na=document.getElementById('navAvatar'); _na.textContent=ini; _na.className='avatar avatar-md av-'+_av;
  const _pa=document.getElementById('profileAvatar'); _pa.textContent=ini; _pa.className='avatar avatar-xl av-'+_av;
  buildAvatarPicker();
  ...
  const _ap=document.getElementById('adminPill'); if(_ap)_ap.style.display=SESSION.isAdmin?'':'none';
}
...
function avatarPreset(userId){ const p=STATE.profiles.find(x=>x.id===userId); return (p&&p.avatar)||'a1'; }
function avatarHTML(name,userId,sizeCls,extra=''){ return `<div class="avatar ${sizeCls} av-${avatarPreset(userId)}" ${extra}>${esc((name||'?')[0]||'?')}</div>`; }
...
function cardHTML(f,saves,i=0){
  ...
  return `<div class="rcard" style="animation-delay:${i*40}ms" data-id="${f.id}" onclick="openPreview(${f.id})">
    ...
    <div class="rcard-tags">
      ...
      ${(f.tags||[]).slice(0,3).map(tg=>`<span class="tag tag-neutral">#${esc(tg)}</span>`).join('')}
    </div>
    ...
    <div class="rcard-footer">
      <div class="rcard-author">
        ${avatarHTML(f.uploaderName,f.uploaderId,'avatar-sm')}
      ...
      <div class="rcard-actions">
        ...
        ${(own||(SESSION&&SESSION.isAdmin))?`<button class="btn btn-danger btn-icon btn-sm" onclick="deleteFile(event,${f.id})" title="Sil">🗑️</button>`:''}
      </div>
    </div>
  </div>`;
}
...
let FIRST_LOAD=true;
const SKEL_HTML='<div class="skel-grid">'+Array(6).fill('<div class="skel"></div>').join('')+'</div>';
async function refreshAll(){
  if(FIRST_LOAD){const _mg=document.getElementById('mainGrid'); if(_mg)_mg.innerHTML=SKEL_HTML;}
  await loadAllData();
  FIRST_LOAD=false;
...
async function reportFile(id){
  const reason=prompt('Bu kaynağı neden bildiriyorsun? (ör: alakasız, telif, spam)');
  if(reason===null)return;
  const {error}=await SB.from('reports').insert({file_id:id,reporter_id:SESSION.userId,reason:(reason||'').slice(0,300)});
  if(error){ toast('❌ Bildirilemedi: '+error.message); return; }
  toast('🚩 Bildirin alındı, teşekkürler.');
}
async function adminResolveReport(rid,delFile,fid){
  if(delFile&&fid){
    const file=STATE.files.find(x=>x.id===fid);
    if(file&&file.file_path){ try{ await SB.storage.from('files').remove([file.file_path]); }catch(_){} }
    await SB.from('files').delete().eq('id',fid);
  }
  await SB.from('reports').update({resolved:true}).eq('id',rid);
  toast(delFile?'🗑️ Dosya silindi, bildirim kapatıldı.':'✅ Bildirim yoksayıldı.');
  buildAdmin();
}
...
function buildAvatarPicker(){
  const el=document.getElementById('avPick'); if(!el||!SESSION)return;
  const cur=SESSION.avatar||'a1', ini=(SESSION.name.trim()[0]||'?').toUpperCase();
  el.innerHTML=['a1','a2','a3','a4','a5'].map(a=>`<div class="av-opt ${a===cur?'sel':''}" onclick="setAvatar('${a}')"><div class="avatar avatar-lg av-${a}">${esc(ini)}</div></div>`).join('');
}
async function setAvatar(a){
  SESSION.avatar=a;
  const {error}=await SB.from('profiles').update({avatar:a}).eq('id',SESSION.userId);
  if(error){ toast('❌ Avatar kaydedilemedi.'); return; }
  const _na=document.getElementById('navAvatar'); if(_na)_na.className='avatar avatar-md av-'+a;
  const _pa=document.getElementById('profileAvatar'); if(_pa)_pa.className='avatar avatar-xl av-'+a;
  buildAvatarPicker();
...
// ═══════════════════════════════════════════════════
// NAVIGATION
const PAGE_INDEX={anasayfa:0,dersler:1,araclar:2,kaynaklar:3,topluluk:4,profil:5,admin:6};
...
async function handleSearch(q){
  ...
  const res=all.filter(f=>{
    ...
    return f.title.toLowerCase().includes(ql)
      ...
      ||(f.tags||[]).some(tg=>tg.toLowerCase().includes(ql));
  });
...
async function toggleSave(e,id){
  e.stopPropagation();
  const isSaved=(STATE.myProfile&&STATE.myProfile.saves||[]).includes(id);
  if(isSaved) await SB.from('saves').delete().eq('file_id',id).eq('user_id',SESSION.userId);
  else await SB.from('saves').insert({file_id:id,user_id:SESSION.userId});
...
async function toggleLike(e,id){
  ...
  const isLiked=f.likes.includes(SESSION.userId);
  if(isLiked) await SB.from('likes').delete().eq('file_id',id).eq('user_id',SESSION.userId);
  else await SB.from('likes').insert({file_id:id,user_id:SESSION.userId});
  toast(isLiked?'👍 Beğeni geri alındı':'👍 Beğendin!');
  await refreshAll();
}
...
async function downloadFile(e,id){
  ...
  if(!f||!f.file_path){toast('❌ Dosya bulunamadı.');return;}
  try{ await SB.rpc('bump_download',{fid:id}); }catch(_){}
  const url=SB.storage.from('files').getPublicUrl(f.file_path,{download:f.fileName||'dosya'}).data.publicUrl;
  const a=document.createElement('a'); a.href=url; a.download=f.fileName||'dosya'; document.body.appendChild(a); a.click(); a.remove();
...
async function openPreview(id){
  ...
  // Build preview frame (Supabase public URL)
  let frame='';
  if(f.file_path){
    const url=SB.storage.from('files').getPublicUrl(f.file_path).data.publicUrl;
    if(['png','jpg','jpeg','gif','webp'].includes(ext))
      frame=`<div class="pv-frame"><img src="${url}" alt=""></div>`;
    else if(ext==='pdf')
      frame=`<div class="pv-frame"><iframe src="${url}"></iframe></div>`;
    else
  ...
  document.getElementById('previewBody').innerHTML=`
    ...
    <div class="pv-meta-row">
      <span>${avatarHTML(f.uploaderName,f.uploaderId,'avatar-sm','style=\'display:inline-flex;vertical-align:middle\'')} <b>${esc(f.uploaderName)}</b></span>
      <span>📅 ${dateStr}</span>
    ...
    <div class="cm-block">
      ...
      <div class="cm-list">
        ${comments.length?comments.slice().reverse().map(c=>{
          ...
          return `<div class="cm">
            ${avatarHTML(c.name,c.userId,'avatar-sm')}
    ...
    <div class="pv-actions">
      ...
      ${own?'':`<button class="btn btn-ghost" onclick="reportFile(${f.id})" title="Şikayet et">🚩 Bildir</button>`}
      ${(own||(SESSION&&SESSION.isAdmin))?`<button class="btn btn-danger" style="margin-left:auto" onclick="deleteFile(event,${f.id});closePreview()">🗑️ Sil</button>`:''}
    </div>`;
...
async function rateFile(id,stars){
  await SB.from('ratings').upsert({file_id:id,user_id:SESSION.userId,score:stars});
  toast(`⭐ ${stars} yıldız verdin!`);
  await refreshAll(); openPreview(id);
}

async function addComment(id){
  const txt=v('cmInput'); if(!txt){toast('❌ Yorum boş olamaz.');return;}
  await SB.from('comments').insert({file_id:id,user_id:SESSION.userId,user_name:SESSION.name,body:txt});
...
async function deleteFile(e,id){
  ...
  const f=STATE.files.find(x=>x.id===id);
  if(f&&f.file_path){ try{ await SB.storage.from('files').remove([f.file_path]); }catch(_){} }
  const {error}=await SB.from('files').delete().eq('id',id);
  if(error){ toast('❌ Silinemedi: '+error.message); return; }
  toast('🗑️ Dosya silindi.'); await refreshAll(); if(document.getElementById('page-admin').classList.contains('active'))buildAdmin();
}
...
function closeUpload(){
  ...
  clearFile();
  ['upTitle','upDesc','upTags'].forEach(id=>document.getElementById(id).value='');
...
async function doUpload(){
  ...
  animBar();
  try{
    const safe=selFile.name.replace(/[^a-zA-Z0-9._-]/g,'_');
    const path=`${SESSION.userId}/${Date.now()}_${safe}`;
    const {error:upErr}=await SB.storage.from('files').upload(path,selFile,{upsert:false});
    if(upErr) throw upErr;
    const tags=v('upTags').split(',').map(x=>x.trim()).filter(Boolean).slice(0,8);
    const {error:insErr}=await SB.from('files').insert({title,description:desc,type,ders,uploader_id:SESSION.userId,uploader_name:SESSION.name,file_path:path,file_name:selFile.name,size:selFile.size,tags});
    if(insErr) throw insErr;
  }catch(err){
    document.getElementById('upBtn').disabled=false;
    document.getElementById('upProg').classList.remove('show');
    toast('❌ Yükleme hatası: '+(err.message||err)); return;
  }
...
async function buildLeaderboard(){
  ...
  el.innerHTML=rows.map((r,i)=>`
    <div class="lb-row ${i===0?'top1':''}">
      ...
      ${avatarHTML(r.name,r.id,'avatar-md')}
      <div style="min-width:0">
...
async function importData(file){
  ...
  toast('ℹ️ İçe aktarma çevrimiçi sürümde devre dışı (veriler artık ortak sunucuda).');
}

// ═══════════════════════════════════════════════════
// ADMIN / MODERASYON
async function buildAdmin(){
  if(!SESSION||!SESSION.isAdmin){ goTo('anasayfa',null); return; }
  await loadAllData();
  document.getElementById('adminUserCount').textContent=STATE.profiles.length;
  document.getElementById('adminFileCount').textContent=STATE.files.length;
  const {data:reps}=await SB.from('reports').select('*').order('created_at',{ascending:false});
  const rel=document.getElementById('adminReports');
  const openReps=(reps||[]).filter(r=>!r.resolved);
  rel.innerHTML=openReps.length?openReps.map(r=>{
    const file=STATE.files.find(x=>x.id===r.file_id);
    const reporter=STATE.profiles.find(x=>x.id===r.reporter_id);
    return `<div class="card card-md" style="display:flex;align-items:center;gap:var(--sp-3);margin-bottom:var(--sp-2);border-color:rgba(239,68,68,.25)">
      <div style="flex:1;min-width:0">
        <div style="font-weight:700">${file?esc(file.title):'(dosya silinmiş)'}</div>
        <div style="font-size:var(--text-xs);color:var(--tx-tertiary);margin-top:3px">Sebep: ${esc(r.reason||'—')} · Bildiren: ${esc(reporter?reporter.name:'?')}</div>
      </div>
      ${file?`<button class="btn btn-ghost btn-sm" onclick="openPreview(${r.file_id})">Gör</button>`:''}
      ${file?`<button class="btn btn-danger btn-sm" onclick="adminResolveReport(${r.id},true,${r.file_id})">Dosyayı Sil</button>`:''}
      <button class="btn btn-ghost btn-sm" onclick="adminResolveReport(${r.id},false,null)">Yoksay</button>
    </div>`;
  }).join(''):'<div class="sec-sub" style="color:var(--tx-tertiary)">Bekleyen bildirim yok. 👍</div>';
  const uel=document.getElementById('adminUsers');
  uel.innerHTML=STATE.profiles.map(p=>{
    const me=p.id===SESSION.userId;
    return `<div class="card card-md" style="display:flex;align-items:center;gap:var(--sp-3);margin-bottom:var(--sp-2)">
      ${avatarHTML(p.name,p.id,'avatar-md')}
      <div style="flex:1;min-width:0">
        <div style="font-weight:700">${esc(p.name||'(isimsiz)')} ${p.is_admin?'<span class="tag tag-brand" style="margin-left:4px">Admin</span>':''} ${p.banned?'<span class="tag tag-red" style="margin-left:4px">Engelli</span>':''} ${me?'<span class="tag tag-neutral" style="margin-left:4px">Sen</span>':''}</div>
        <div style="font-size:var(--text-xs);color:var(--tx-tertiary)">${p.year==='mezun'?'Mezun':(p.year?p.year+'. Sınıf':'—')} · ${(p.saves||[]).length} kayıt</div>
      </div>
      ${me?'':`<button class="btn ${p.banned?'btn-ghost':'btn-danger'} btn-sm" onclick="adminBan('${p.id}',${p.banned?'false':'true'})">${p.banned?'Engeli Kaldır':'Engelle'}</button>`}
    </div>`;
  }).join('')||'<div class="sec-sub" style="color:var(--tx-tertiary)">Kullanıcı yok.</div>';
  const fel=document.getElementById('adminFiles');
  const files=STATE.files.slice().sort((a,b)=>b.createdAt-a.createdAt);
  fel.innerHTML=files.map(f=>{
    const cfg=TYPE_CONFIG[f.type]||TYPE_CONFIG.diger;
    return `<div class="card card-md" style="display:flex;align-items:center;gap:var(--sp-3);margin-bottom:var(--sp-2)">
      <div style="flex:1;min-width:0">
        <div style="font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(f.title)}</div>
        <div style="font-size:var(--text-xs);color:var(--tx-tertiary);margin-top:3px"><span class="tag ${cfg.cls}">${cfg.label}</span> ${esc(getDersName(f.ders))} · ${esc(f.uploaderName||'?')} · ⬇️ ${f.downloads||0} · 👍 ${f.likes.length}</div>
      </div>
      <button class="btn btn-ghost btn-sm" onclick="openPreview(${f.id})">Gör</button>
      <button class="btn btn-danger btn-sm" onclick="deleteFile(event,${f.id})">🗑️</button>
    </div>`;
  }).join('')||'<div class="sec-sub" style="color:var(--tx-tertiary)">Henüz yükleme yok.</div>';
}
async function adminBan(uid,ban){
  const {error}=await SB.from('profiles').update({banned:ban}).eq('id',uid);
  if(error){ toast('❌ İşlem başarısız: '+error.message); return; }
  toast(ban?'🚫 Kullanıcı engellendi.':'✅ Engel kaldırıldı.');
  buildAdmin();
}
